...
// Avisa por e-mail que o certificado está disponível. Mesmo mecanismo/secrets
// de supabase/functions/enviar-convite e enviar-pesquisa.
...
const DEFAULT_MENSAGEM = "Seu certificado de participação já está disponível! Clique no botão abaixo para acessar.";

function gerarTemplateHTML({ event, bannerUrl, certificadoUrl, assunto, mensagem, corCabecalho, corRodape, corBotao, ctaTexto }: any) {
...
<title>${assunto || `Certificado disponível — ${event.nome}`}</title>
</head>
      ...
      <tr><td style="padding:40px 48px;">
        ...
        <table cellpadding="0" cellspacing="0" style="margin:0 auto 20px;">
          <tr><td align="center" style="border-radius:8px;background:${corCta};">
            <a href="${certificadoUrl}" style="display:inline-block;padding:16px 40px;font-size:16px;font-weight:700;color:#c9a84c;text-decoration:none;letter-spacing:0.5px;">
              ${ctaTexto || "Ver certificado →"}
            </a>
          </td></tr>
        </table>
        <p style="font-size:13px;color:#a0aec0;text-align:center;margin:0;">
          Se o botão não funcionar, acesse: <a href="${certificadoUrl}" style="color:#0a1f40;">${certificadoUrl}</a>
        </p>
      </td></tr>
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { destinatarios, event, bannerUrl, certificadoUrl, assunto, mensagem, corCabecalho, corRodape, corBotao, ctaTexto } = await req.json();
    ...
    const html = gerarTemplateHTML({ event: event || {}, bannerUrl, certificadoUrl, assunto, mensagem, corCabecalho, corRodape, corBotao, ctaTexto });
    ...
    for (const dest of destinatarios) {
      try {
        await enviarComRetry({
          ...
          to: dest.email,
          subject: assunto || `Certificado disponível — ${event?.nome || "Evento"}`,
          mimeContent,
        });
        sent.push(dest.id);
      } catch (err) {
