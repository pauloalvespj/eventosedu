...
export function Inscricoes() {
  ...
  const filtrados = participantes.filter(p => {
    ...
    const passaBusca = !q || p.nome?.toLowerCase().includes(q) || p.cpf?.includes(q) || p.instituicao?.toLowerCase().includes(q) || p.cargo?.toLowerCase().includes(q) || p.email?.toLowerCase().includes(q);
    const passaRole = filtroRole === "todos" || p.role === filtroRole;
    return passaBusca && passaRole;
  });
  ...
  function salvar() {
    ...
    if (modalPart === "new") {
      const novo = { ...formPart, role, id: `local-${Date.now()}`, credenciado: false,
  ...
  const totalPalestrantes  = participantes.filter(p => p.role === "palestrante").length;
  ...
  return (
    <div>
      <div className="admin-topbar">
        <div>
          <h1>Inscrições</h1>
          <p>{totalParticipantes} participante{totalParticipantes !== 1 ? "s" : ""} · {totalPalestrantes} palestrante{totalPalestrantes !== 1 ? "s" : ""}</p>
        </div>
        <button className="btn btn-primary" onClick={() => { setFormPart({ nome:"", cpf:"", email:"", instituicao:"", cargo:"", role:"participante" }); setModalPart("new"); }}>
      ...
      <div className="table-wrap">
        <div className="table-header">
          ...
          <div style={{ display:"flex", gap:"0.5rem", alignItems:"center" }}>
            <select className="search-input" style={{ width:"auto", borderRadius:"var(--radius-sm)" }} value={filtroRole} onChange={e => setFiltroRole(e.target.value)}>
        ...
        <table style={{ width:"100%", fontSize:"0.83rem" }}>
          <thead><tr><th>Nome</th><th>CPF</th><th>Instituição / Cargo</th><th>E-mail</th><th>Tipo</th><th>Status</th><th style={{ width:88 }}>Ações</th></tr></thead>
          <tbody>
            {filtrados.map(p => (
              <tr key={p.id}>
                <td><div style={{ display:"flex", alignItems:"center", gap:8 }}>
                  {p.foto_url
                    ...
                    : <div style={{ width:28, height:28, borderRadius:"50%", background:"var(--navy)", color:"#fff", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"0.7rem", fontWeight:700, flexShrink:0 }}>{p.foto_iniciais}</div>
                  }
      ...
      <Modal show={!!modalPart} onClose={() => setModalPart(null)} title={modalPart === "new" ? "Novo Inscrito" : "Editar Inscrito"}>
        ...
        <div className="form-grid">
          ...
          <div className="form-group">
            ...
            <select className="form-input" value={formPart.role||"participante"} onChange={e => setFormPart(f=>({...f,role:e.target.value}))}>
              {ROLE_OPTS.map(r => <option key={r.value} value={r.value}>{r.label}</option>)}
            </select>
          </div>
        </div>
